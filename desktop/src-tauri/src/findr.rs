...
/// Async-native single scan: async fetch (NO nested `Runtime::new()` — that panics inside the
/// async runtime and was the FINDR-hang root cause) followed by the CPU-bound scan math.
/// `engine::scan_frame` is microsecond-scale over already-fetched data, so it runs inline.
async fn attempt_scan(
    symbol: String,
    timeframe: String,
    market_ok: bool,
) -> Result<engine::ScanResult, String> {
    let d = fetch::fetch_scan_async(&symbol, &timeframe).await?;
    if d.len() < 60 {
        return Err(format!("only {} {} candles for {}", d.len(), timeframe, symbol));
    }
    Ok(engine::scan_frame(&d, market_ok, 0.005))
}

/// Headless debug runner for the `--findr` CLI mode. Runs the SAME universe + bounded-concurrency
/// scan as the `findr` Tauri command, but prints each ticker to STDOUT as it finishes (and status
/// to STDERR) instead of emitting Tauri events — so a run's activity is visible in a terminal and
/// the async concurrency path can be exercised without the GUI.
pub async fn run_headless(start: char, end: char, timeframe: &str, limit: Option<usize>) {
    let (lo, hi) = if start <= end { (start, end) } else { (end, start) };
    let tf = if timeframe == "daily" { "daily" } else { "weekly" };
    eprintln!("[findr] building ticker universe…");
    let uni = match universe().await {
        Ok(u) => u,
        Err(e) => { eprintln!("[findr] universe error: {e}"); return; }
    };
    let mut symbols: Vec<(String, String)> = uni
        .into_iter()
        .filter(|(s, _)| s.chars().next().is_some_and(|c| c >= lo && c <= hi))
        .collect();
    if let Some(n) = limit {
        if n > 0 && n < symbols.len() { symbols.truncate(n); }
    }
    let total = symbols.len();
    eprintln!("[findr] {total} tickers in [{lo}-{hi}] tf={tf} concurrency={CONCURRENCY}");
    let t_market = std::time::Instant::now();
    let market_ok = fetch::market_ok_async(tf).await;
    eprintln!("[findr] market_ok={market_ok} (SPY fetch {}ms)", t_market.elapsed().as_millis());
    ...
    for (symbol, _name) in symbols {
        ...
        set.spawn(async move {
            ...
            let mut r = attempt_scan(symbol.clone(), tf.clone(), market_ok).await.ok();
            for &b in RETRY_BACKOFFS_MS.iter() {
                if r.is_some() { break; }
                tokio::time::sleep(Duration::from_millis(b)).await;
                r = attempt_scan(symbol.clone(), tf.clone(), market_ok).await.ok();
            }
            let n = scanned.fetch_add(1, Ordering::Relaxed) + 1;
            let ms = ts.elapsed().as_millis();
            match r {
                Some(sr) => {
                    let is_buy = sr.verdict.starts_with("BUY");
                    if is_buy { found.fetch_add(1, Ordering::Relaxed); }
                    println!(
                        "[{n:>4}/{total:<4}] {symbol:<6} {:<34} R={:>7.2} up={:>7.1}% {ms:>5}ms{}",
                        sr.verdict, sr.r_multiple, sr.upside_pct,
                        if is_buy { "  <== BUY" } else { "" }
                    );
                }
                None => println!("[{n:>4}/{total:<4}] {symbol:<6} SKIP (fetch error){:>40}{ms:>5}ms", ""),
            }
        });
    }
    while set.join_next().await.is_some() {}
    eprintln!(
        "[findr] DONE — scanned {}/{total}, found {}, total {}s",
...
pub async fn findr(
    ...
    let mut symbols: Vec<(String, String)> = uni
        ...
        .collect();
    // Optional scan cap: limit the number of tickers scanned (None / 0 = no cap).
    if let Some(n) = limit {
        if n > 0 && (n as usize) < symbols.len() {
            symbols.truncate(n as usize);
        }
    }
    let total = symbols.len();
